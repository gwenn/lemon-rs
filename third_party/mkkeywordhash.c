...
int main(int argc, char **argv){
  ...
  ** number of collisions */
  ...
  printf("#[allow(clippy::byte_char_slices)]\n");
  printf("#[expect(non_upper_case_globals)]\n");
  printf("static zKWText: [u8; %d] = [\n", nChar);
  zKWText[nChar] = 0;
  for(i=j=0; i<k; i++){
    if( j==0 ){
      printf("  ");
  ...
  printf("#[expect(non_upper_case_globals)]\n");
  printf("static aKWNext: [usize; %d] = [0,\n", nKeyword+1);
  for(i=j=0; i<nKeyword; i++){
    if( j==0 ) printf("  ");
  ...
  printf("#[expect(non_upper_case_globals)]\n");
  printf("static aKWLen: [usize; %d] = [0,\n", nKeyword+1);
  for(i=j=0; i<nKeyword; i++){
    if( j==0 ) printf("  ");
  ...
  printf("#[expect(non_upper_case_globals)]\n");
  printf("static aKWOffset: [usize; %d] = [0,\n", nKeyword+1);
  for(i=j=0; i<nKeyword; i++){
    if( j==0 ) printf("  ");
  ...
  printf("#[expect(non_upper_case_globals)]\n");
  printf("static aKWCode: [TokenType; %d] = [TokenType::TK_EOF,\n", nKeyword+1);
  for(i=j=0; i<nKeyword; i++){
    char *zToken = aKeywordTable[i].zTokenType;
    if( j==0 ) printf("  ");
    printf("TokenType::%s,%*s", zToken, (int)(14-strlen(zToken)), "");
    j++;
    if( j>=5 ){
      printf("\n");
      j = 0;
  ...
  printf("#[expect(non_snake_case, clippy::identity_op)]\n");
  printf("fn keywordCode(z: &[u8]) -> Option<TokenType> {\n");
  printf("  let n = z.len();\n");
  printf("  debug_assert!( n>=2 );\n");
  printf("  let i = ((charMap(z[0])*%d) %c", HASH_C0, HASH_CC);
  printf(" (charMap(z[n-1])*%d) %c", HASH_C1, HASH_CC);
  printf(" (n*%d)) %% %d;\n", HASH_C2, bestSize);
  printf("  let mut i=aKWHash[i];\n");
  printf("  while i>0 {\n");
  printf("    let mut _first = true;\n");
  printf("    loop {\n");
  printf("    if !_first { break }\n");
  printf("    _first = false;\n");
  //printf("  for(i=(int)aKWHash[i]; i>0; i=aKWNext[i]){\n");
  printf("    if aKWLen[i]!=n { continue }\n");
  printf("    let zKW = &zKWText[aKWOffset[i]..];\n");
  //printf("#ifdef SQLITE_ASCII\n");
  printf("    if (z[0]&!0x20)!=zKW[0] { continue }\n");
  printf("    if (z[1]&!0x20)!=zKW[1] { continue }\n");
  printf("    let mut j = 2;\n");
  printf("    while j<n && (z[j]&!0x20)==zKW[j] { j+=1; }\n");
  //printf("#endif\n");
  //printf("#ifdef SQLITE_EBCDIC\n");
  printf("    if z[0].to_ascii_uppercase()!=zKW[0] { continue }\n");
  printf("    if z[1].to_ascii_uppercase()!=zKW[1] { continue }\n");
  printf("    j = 2;\n");
  printf("    while j<n && z[j].to_ascii_uppercase()==zKW[j] { j+=1; }\n");
  //printf("#endif\n");
  ...
  //}
  printf("    return Some(aKWCode[i]);\n");
  printf("    }\n");
  printf("    i=aKWNext[i];\n");
  printf("  }\n");
  printf("  None\n");
  printf("}\n");
  printf("#[expect(non_snake_case)]\n");
  printf("fn charMap(b: u8) -> usize {\n");
  printf("  b.to_ascii_lowercase() as usize\n");
  printf("}\n");
  /*
